...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	public AnnotationConfigApplicationContext() {
		/**
		 *  AnnotatedBeanDefinitionReader用来读取加了注解的bean
		 */
		this.reader = new AnnotatedBeanDefinitionReader(this);
		/**
		 * 扫描包或类,然后转化为BeanDefinition
		 * 我们扫描包工作不是scanner这个对象来完成的
	...
	public AnnotationConfigApplicationContext(Class<?>... annotatedClasses) {
		/**
		 * 调用默认的无参构造时,beanFactory会注册5个对象信息放入beanDefinitionMap中
		 * 这5个beanDefinition实现了BeanPostProcessor接口
		 */
		this();
		/**
		 * 注册配置类的时候,注册ConfigurationClassPostProcessor对象信息到beanDefinitionMap中
		 * 此对象实现了BeanFactoryPostProcessor 不同于上面那一个对象
		 * ConfigurationClassPostProcessor  spring核心配置类  没有之一
		 */
